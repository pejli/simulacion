...
resultadox<-data.frame(Réplicas=integer(),Objetos=double(),Tiempo=double(), Método=character(), Objetivo=double(), Beneficio=double())
...
######INICIO PARALLEL
 ...
 resultadox<-rbind(resultadox, data.frame(Réplicas=replicas,Objetos=n,Tiempo=final-inicio, Método="Exacto", Objetivo=optimo, Beneficio=optimo/(final-inicio)))
 ...
 inicio<-as.numeric(Sys.time())
  for (iter in 1:tmax) {
    ...
    final<-as.numeric(Sys.time())
    resultadox<-rbind(resultadox, data.frame(Réplicas=replicas,Objetos=n,Tiempo=final-inicio, Método="Génetico", Objetivo=max(mejores), Beneficio=(max(mejores)/(final-inicio))))
 }
}
stopCluster(cluster)