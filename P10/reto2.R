...
#DATA.FRAME
datos<-data.frame(Réplica=integer(),Optimo=double(), 
                  Método=character(),Objetivo=double())
...
for (replicas in 1:3) {
  ...
  for (iter in 1:tmax) {
    ...
    pp<-t(unlist(parSapply(cluster,1:tam,muttacion)))
    pp<-pp[pp[,1] < 2,]
    p<-rbind(p,pp)
    ...
    p<-cbind(p,obj)
    suma<-sum(p$obj)
    p<-cbind(p,proba=obj/suma)
    tam <- dim(p)[1]
    #FINAL
    k=100
    mantener <- order(-p[, (n + 2)], -p[, (n + 1)])
    primeros<- mantener[1:k]
    mantener1<-sample(mantener[(k+1):tam], init-k, replace=FALSE,prob=p$proba[(k+1):tam])
    p<-rbind(p[primeros,],p[mantener1,])
    tam <- dim(p)[1]
    assert(tam == init)
